...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!result.success || !result.data) {
      trackRouteRequest(req, { tool: 'watermark-pdf', fileSizeB: file.size, format: 'pdf', success: false, durationMs: Date.now() - start, errorMsg: result.error ?? 'watermark failed' })
    ...
    trackRouteRequest(req, { tool: 'watermark-pdf', fileSizeB: file.size, format: 'pdf', success: true, durationMs: Date.now() - start })
    ...
    trackRouteRequest(req, { tool: 'watermark-pdf', fileSizeB: files[0]?.size, format: 'pdf', success: false, durationMs: Date.now() - start, errorMsg: err instanceof Error ? err.message : 'unknown' })
