...
export function SplitPdfClient() {
  ...
  // Poll job status every second until done or failed
  useEffect(() => {
    if (!jobId) return
    ...
    const poll = async () => {
      try {
        ...
        if (typeof data.progress === 'number' && data.progress > 0) {
          // Map job progress (0–100) into the processing stage (55–90%)
          const mapped = Math.round((data.progress / 100) * 35)
          progress.stageProcessing(mapped, 'Splitting pages...')
        }
  ...
  const handleSplit = async () => {
    ...
    try {
      ...
      const data = await res.json() as { jobId: string; pollUrl: string }
  ...
  const downloadFilename = result?.fileName ?? (isZip ? 'toolify-split.zip' : 'toolify-split.pdf')
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <button
                onClick={() => { setMode('all'); setResult(null) }}
                disabled={isProcessing}
                className={cn(
          ...
          {mode === 'range' && (
            <div>
              ...
              <input
                type="text"
                value={rangeStr}
                onChange={(e) => { setRangeStr(e.target.value); setResult(null) }}
          ...
          {result && progress.status === 'completed' && (
            <div className="bg-green-50 border border-green-200 rounded-xl p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-green-700">
                    {isZip ? t('split.multiplePackaged') : t('split.singleExtracted')}
                  </p>
                </div>
              </div>
              <a
              ...
              >
                ...
                {isZip ? t('split.downloadZip') : t('split.downloadPdf')}
              </a>
            </div>
          )}
